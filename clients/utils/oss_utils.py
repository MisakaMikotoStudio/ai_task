...
def download_image_to_file(config: OssConfig, oss_path: str, local_path: str):
    ...
    cos_config_kwargs = {'Region': config.region, 'SecretId': config.secret_id, 'SecretKey': config.secret_key}
    # STS 临时凭证需要传入 Token
    ...
    response = client.get_object(Bucket=config.bucket, Key=oss_path)
