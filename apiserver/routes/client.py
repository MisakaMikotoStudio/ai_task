...
@client_bp.route('/<int:client_id>/infrastructure', methods=['GET'])
def get_infrastructure_api(client_id):
    """
    获取客户端全量基础设施配置（云服务器、域名、数据库、支付、对象存储）
    ...
    data = get_client_infrastructure(client_id=client_id, user_id=user_id)
    return jsonify({'code': 200, 'data': data})


@client_bp.route('/<int:client_id>/servers', methods=['PUT'])
def save_servers_api(client_id):
    """
    保存云服务器配置（先进行 SSH 连通性校验）

    Request Body:
        {
            "test": {"name": "root", "password": "xxx", "ip": "1.2.3.4"},
            "prod": {"name": "root", "password": "xxx", "ip": "5.6.7.8"}
        }

    Response:
        成功 (200): {"code": 200, "message": "保存成功"}
        SSH失败 (400): {"code": 400, "message": "SSH校验失败说明"}
    """
    user_id = request.user_info.id
    if not get_client_by_id(client_id=client_id, user_id=user_id):
        return jsonify({'code': 400, 'message': '客户端不存在或无权限'}), 400
    ...
    # SSH 连通性校验
    ssh_ok, ssh_err = check_servers_ssh(body)
    if not ssh_ok:
        return jsonify({'code': 400, 'message': ssh_err}), 400
...
@client_bp.route('/<int:client_id>/domains', methods=['PUT'])
def save_domains_api(client_id):
    """
    保存域名配置
...
@client_bp.route('/<int:client_id>/databases', methods=['PUT'])
def save_databases_api(client_id):
    """
    保存数据库配置（支持多条）
    ...
    Request Body:
        {
            "test": [
                {"db_type": "mysql", "host": "127.0.0.1", "port": 3306,
                 "username": "root", "password": "xxx", "db_name": "mydb"}
            ],
            "prod": [...]
        }
    """
...
@client_bp.route('/<int:client_id>/payments', methods=['PUT'])
def save_payments_api(client_id):
    """
    保存支付配置
    ...
    Request Body:
        {
            "test": {
                ...
                "alipay_public_key": "...", "notify_url": "...",
                "return_url": "...", "gateway": "...", "app_encrypt_key": "..."
            },
            "prod": {...}
        }
    """
...
@client_bp.route('/<int:client_id>/oss', methods=['PUT'])
def save_oss_api(client_id):
    """
    保存对象存储配置
    ...
    Request Body:
        {
            "test": {
                ...
                "region": "ap-guangzhou", "bucket": "xxx-12345678",
                "base_url": "https://xxx.cos.ap-guangzhou.myqcloud.com"
            },
            "prod": {...}
        }
    """
