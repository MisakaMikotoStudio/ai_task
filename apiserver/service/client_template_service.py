...
def generate_default_database(user_id: int, config) -> dict:
    ...
    try:
        admin_conn = pymysql.connect(
            ...
            user=config.admin_username, password=config.admin_password,
            connect_timeout=10,
        )
        ...
        if app_username:
            cursor.execute(f"GRANT ALL PRIVILEGES ON `{db_name}`.* TO %s@'%%'", (app_username,))
            cursor.execute("FLUSH PRIVILEGES")

        admin_conn.commit()

        logger.info("Default database created: db_name=%s, user_id=%s, host=%s", db_name, user_id, config.url)
        ...
        logger.error("Failed to create default database: user_id=%s, error=%s", user_id, str(e))
