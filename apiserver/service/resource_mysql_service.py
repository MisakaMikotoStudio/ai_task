...
# -*- coding: utf-8 -*-
"""
MySQL 资源服务层 —— 业务逻辑（用户数据库管理、命名规则）

底层阿里云 RDS SDK 操作委托给 utils.mysql_utils
"""
...
def list_databases(resource: Resource, user_id: int) -> List[Dict]:
    ...
    try:
        databases = describe_databases(client=client, instance_id=instance_id)
...
def create_database_with_name(resource: Resource, user_id: int, db_name: str) -> Dict:
    ...
    Returns:
        {"db_name", "account_name", "account_password", "instance_url", "port"}
    """
    client, instance_id = _build_rds_client(resource=resource)
    extra = resource.get_raw_extra()
    ...
    try:
        # 1. 创建数据库
        create_database(client=client, instance_id=instance_id, db_name=db_name)
        logger.info(
        ...
        account_password = generate_password(length=16)
...
def create_database_for_user(resource: Resource, user_id: int) -> Dict:
    ...
    Returns:
        {"db_name", "account_name", "account_password", "instance_url"}
    """
    client, instance_id = _build_rds_client(resource=resource)
    extra = resource.get_raw_extra()
    instance_url = extra.get('url', '')
    ...
    try:
        ...
        databases = describe_databases(client=client, instance_id=instance_id)
        existing_dbs = {db.dbname for db in databases}
